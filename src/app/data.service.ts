...
export class DataService {
  ...
  userRegister(userRegi){
    const myheader = new HttpHeaders().set('Content-Type', 'application/x-www-form-urlencoded')
    const body = new HttpParams().set('action','registration').set('name', 'name').set('email','email').set('mobile','mobile').set('password',JSON.stringify(userRegi));
    return this._http.post(environment.api_url, body,{headers:myheader });
  }
